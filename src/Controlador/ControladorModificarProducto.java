...
public class ControladorModificarProducto implements ActionListener {
    ...
    private void cargarDatosProducto() {
        if (datosProducto != null && datosProducto.length >= 9) {
            ventanaProducto.getTidProducto().setText(datosProducto[0]);  // ID
            ventanaProducto.getTsku().setText(datosProducto[1]);         // SKU
            ventanaProducto.getTproducto().setText(datosProducto[2]);    // Nombre
            // La categoría se seleccionará cuando se carguen las categorías
            // El proveedor se seleccionará cuando se carguen los proveedores
            ventanaProducto.getTprecioventa().setText(datosProducto[4]); // Precio
            ventanaProducto.getTmarca().setText(datosProducto[5]);       // Marca
            ventanaProducto.getTstock().setText(datosProducto[6]);       // Stock
            ventanaProducto.getTdescripcion().setText(datosProducto[7]); // Descripción
        }
    }
    /**
    ...
    private String[] obtenerValoresFormulario() {
        ...
        String idCategoria = cat.getId();
            ...
            return new String[] {id, sku, nombre, idCategoria, precioVenta, marca, stock, descripcion,idProveedor};
    }
        ...
        // Acción del botón Guardar
            ...
            try {
                ...
                boolean resultadoCategoria = bd.modificar("Productos", "idcategoria", valores[3], "idProducto = '" + idProducto + "'");
                                boolean resultadoProveedor = bd.modificar("Productos", "idproveedor", valores[8], "idProducto = '" + idProducto + "'");
                                boolean resultadoPrecio = bd.modificar("Productos", "precio_venta", valores[4], "idProducto = '" + idProducto + "'");
                                boolean resultadoMarca = bd.modificar("Productos", "marca", valores[5], "idProducto = '" + idProducto + "'");
                                boolean resultadoStock = bd.modificar("Productos", "stock", valores[6], "idProducto = '" + idProducto + "'");
                                boolean resultadoDesc = bd.modificar("Productos", "descripcion", valores[7], "idProducto = '" + idProducto + "'");
