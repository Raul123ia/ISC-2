...
public class ControladorCaja implements ActionListener {
        ...
        try {
            ...
            if (padre != null && padre.getEscritorio() != null) {
                ...
                try {
                    ...
                    System.out.println("Error al seleccionar ventana: " + ex.getMessage());
                }
            }
    ...
    public ControladorCaja(ControladorMenu controladorMenu) {
        try {
            ...
            if (padre != null && padre.getEscritorio() != null) {
                ...
                // Intentar seleccionar la ventana (enfocarla)
                try {
                    vistaCaja.setSelected(true);
                } catch (java.beans.PropertyVetoException ex) {
                    System.out.println("Error al seleccionar ventana: " + ex.getMessage());
    ...
    private void cargarProductos() {
        ...
        try {
            ...
            bd = new BaseDatos();
            ArrayList<String[]> lista = bd.consultar("Productos", null, null);
    ...
    private void buscarProducto(String codigo) {
        ...
        if (producto == null) {
            ...
            try {
                ...
                if (!resultados.isEmpty() && resultados.get(0).length >= 4) {
                    String[] datos = resultados.get(0);
                    producto = new Producto();
                    producto.setIdProducto(datos[0]);
                    producto.setProducto(datos[1]);
                    try {
                        // Verificar si el valor es numérico antes de convertir
                        if (esNumerico(datos[2])) {
                            producto.setPrecioVenta(Double.parseDouble(datos[2]));
                        } else {
                            producto.setPrecioVenta(0.0);
                            System.out.println("Advertencia: Precio no numérico para producto " + datos[1] + ": " + datos[2] + ". Se asignó 0.0");
                        }
                        
                        if (esNumerico(datos[3])) {
                            producto.setStock(Integer.parseInt(datos[3]));
                        } else {
                            producto.setStock(0);
                            System.out.println("Advertencia: Stock no numérico para producto " + datos[1] + ": " + datos[3] + ". Se asignó 0");
                        }
                    } catch (NumberFormatException e) {
    ...
    private void registrarVenta(String idTipoVenta) {
        ...
        try {
            ...
            // Obtener el próximo ID secuencial para la venta
            ArrayList<String[]> resultadoVenta = bd.consultar("Ventas", "MAX(CAST(idVenta AS INT)) AS ultimoID", "ISNUMERIC(idVenta) = 1");
            int nuevoIdVenta = 1;
            ...
            if (ventaRegistrada) {
                ...
                for (int i = 0; i < modelo.getRowCount(); i++) {
                    try {
                        ...
                        // Obtener el próximo ID secuencial para detalle de venta
                        ArrayList<String[]> resultadoDetalle = bd.consultar("DetalleVentas", "MAX(CAST(idDetalleVenta AS INT)) AS ultimoID", "ISNUMERIC(idDetalleVenta) = 1");
    ...
    private void corregirPreciosNoNumericos() {
        try {
            BaseDatos bd = new BaseDatos();
            ArrayList<String[]> productos = bd.consultar("Productos", null, null);
            
            if (productos != null) {
                for (String[] producto : productos) {
                    String idProducto = producto[0];
                    String sku = producto[1];
                    String nombre = producto[2];
                    String precioStr = producto[4];
                    
                    // Verificar si el precio es numérico
                    try {
                        if (precioStr == null || precioStr.trim().isEmpty()) {
                            System.out.println("Corrigiendo: Precio vacío para producto " + sku + ": " + nombre);
                            ...
                            // Si el formato era diferente, actualizar
                            if (!producto[4].equals(String.valueOf(precio))) {
                                System.out.println("Corrigiendo formato: Precio para producto " + sku + ": " + nombre);
                                bd.modificar("Productos", "precio_venta", String.valueOf(precio), "idProducto = '" + idProducto + "'");
                            }
                        }
                    } catch (NumberFormatException e) {
                        System.out.println("Corrigiendo: Precio no numérico para producto " + sku + ": " + nombre);
                        bd.modificar("Productos", "precio_venta", "0.0", "idProducto = '" + idProducto + "'");
                    }
                }
            }
            ...
            System.err.println("Error al corregir precios: " + e.getMessage());
        }
    }
