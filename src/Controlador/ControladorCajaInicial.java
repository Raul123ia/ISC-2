...
public class ControladorCajaInicial implements ActionListener {
    ...
    private void abrirCaja() {
        try {
            ...
            // Generar ID único para la apertura (usando timestamp)
            String id = String.valueOf(System.currentTimeMillis());
            
            // Registrar apertura de caja en la base de datos
            BaseDatos bd = new BaseDatos();
            String[] valores = {id, fecha, hora, String.valueOf(montoInicial), "1", "", ""}; // 1 = Abierta
            boolean resultado = bd.insertar("CajaAperturas", "id,fecha,hora,monto_inicial,estado,monto_final,hora_cierre", valores);
            bd.cerrarConexion();
            ...
            if (resultado) {
                ...
                try {
                    ...
                    try {
                        ...
                        ventanaCaja.setLocation(50, 50); // Posición fija para mayor seguridad
                        ventanaCaja.toFront();
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(controladorMenu.getVentana().getFrame(),
