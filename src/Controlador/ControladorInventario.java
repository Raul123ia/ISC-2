...
public class ControladorInventario implements ActionListener {
    ...
    private void configurarEventos() {
        ...
        ventana.getBbuscarNombre().addActionListener(this);
        ventana.getBbuscarId().addActionListener(this);
    ...
    private void cargarProductos() {
        ...
        try {
            bd = new BaseDatos();
            ArrayList<String[]> lista = bd.consultar("Productos", null, "activo = 1");
    ...
    private void buscarProducto(boolean porCodigo) {
        String textoBusqueda = ventana.getTbuscar().getText().trim().toLowerCase();
        if (textoBusqueda.isEmpty()) {
            sorter.setRowFilter(null); // Mostrar todos si no hay texto
            return;
        }
        ...
        if (porCodigo) {
            // Filtrar por ID o SKU (columnas 0 y 1)
            filtro = RowFilter.regexFilter("(?i)" + textoBusqueda, 0, 1);
        } else {
            // Filtrar por nombre de producto (columna 2)
            filtro = RowFilter.regexFilter("(?i)" + textoBusqueda, 2);
        }
    ...
    private void buscarPorNombre() {
        ...
        try {
            BaseDatos bd = new BaseDatos();
            ArrayList<String[]> resultado = bd.consultar("Productos", "*", "producto LIKE '%" + nombre + "%' AND activo=1");
            bd.cerrarConexion();
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == ventana.getBsalida()) {
        	...
        	buscarPorNombre();// Buscar por código
            ...
            buscarProducto(false); // Buscar por nombre
    ...
    try {
        ...
        if (fechaInicio != null && !fechaInicio.isEmpty()) {
            ...
            condicion.append("CONVERT(DATE, fecha) >= '").append(fechaInicio).append("'");
        }
        
        if (fechaFin != null && !fechaFin.isEmpty()) {
            if (condicion.length() > 0) condicion.append(" AND ");
            condicion.append("CONVERT(DATE, fecha) <= '").append(fechaFin).append("'");
        }
    ...
    private Producto consultarProducto(String idProducto) {
        try {
            ...
            ArrayList<String[]> resultados = bd.consultar("Productos", 
                "idProducto, producto, descripcion, precio_venta, stock, idcategoria, marca, sku, activo", consulta);
            ...
            try {
                if (datos[3] != null && !datos[3].isEmpty()) {
                    producto.setPrecioVenta(Double.parseDouble(datos[3]));
                }
            } catch (NumberFormatException e) {
            ...
            try {
                if (datos[4] != null && !datos[4].isEmpty()) {
                    producto.setStock(Integer.parseInt(datos[4]));
                }
            } catch (NumberFormatException e) {
            ...
            if (datos.length > 5 && datos[5] != null) {
                producto.setIdCategoria(datos[5]);
            }
            
            if (datos.length > 6 && datos[6] != null) {
                producto.setMarca(datos[6]);
            }
            
            if (datos.length > 7 && datos[7] != null) {
                producto.setSku(datos[7]);
            }
            
            // Establecer el estado activo del producto
            if (datos.length > 8 && datos[8] != null) {
                producto.setActivo("1".equals(datos[8]) || "true".equalsIgnoreCase(datos[8]));
            } else {
